...
void main() {
	...
	// Get the image
	vec3 image = texture2D( inputTexture, vec2(suv.x, suv.y) ).xyz;
  ...
  gl_FragColor.xyz = oldImage * (vI + 0.5);

	// Add some grain (thanks, Jose!)
	#ifdef GRAIN
  gl_FragColor.xyz *= (1.5+(rand(uv+t*.01)-.2)*.15);
  #endif

	gl_FragColor.a = texture2D( inputTexture, vec2(suv.x, suv.y) ).a;
}
